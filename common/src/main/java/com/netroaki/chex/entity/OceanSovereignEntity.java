...
public class OceanSovereignEntity extends Monster {
  private static final EntityDataAccessor<Integer> PHASE =
      SynchedEntityData.defineId(OceanSovereignEntity.class, EntityDataSerializers.INT);
  private static final EntityDataAccessor<Boolean> IS_CHARGING =
      SynchedEntityData.defineId(OceanSovereignEntity.class, EntityDataSerializers.BOOLEAN);
        ...
        if (this.floatDuration-- <= 0) {
          ...
          if (this.canReach(vec3d, (int) Math.ceil(d0))) {
            this.sovereign.setDeltaMovement(
                this.sovereign.getDeltaMovement().add(vec3d.scale(0.1D)));
          } else {
    ...
    public void tick() {
      ...
      if (++this.chargeTime >= 20) {
        ...
        if (this.sovereign.distanceToSqr(target) < 4.0D) {
          target.hurt(
              this.sovereign.damageSources().mobAttack(this.sovereign),
