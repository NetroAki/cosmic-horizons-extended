...
public class ShaiHuludEntity extends Monster {
    private static final EntityDataAccessor<Integer> PHASE = SynchedEntityData.defineId(ShaiHuludEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> IS_EMERGING = SynchedEntityData.defineId(ShaiHuludEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> IS_SUBMERGING = SynchedEntityData.defineId(ShaiHuludEntity.class, EntityDataSerializers.BOOLEAN);
        ...
        if (!this.level().isClientSide) {
            ...
            if (this.isEmerging()) {
                ...
                if (this.tickCount % 20 == 0) {
                    this.level().levelEvent(2001, this.blockPosition(), net.minecraft.world.level.block.Blocks.SAND.defaultBlockState().getBlock().defaultBlockState().getBlock().getId());
                }
    ...
    protected void pushEntities() {
        ...
        if (this.isEmerging()) {
            ...
            this.level().getEntities(this, aabb).forEach(entity -> {
                if (entity instanceof LivingEntity && !(entity instanceof ShaiHuludEntity)) {
                    Vec3 pushVec = entity.position().subtract(this.position()).normalize().scale(2.0D);
