...
public final class PlanetOverridesCore {
  /**
   * Represents a set of overrides that can be applied to a planet definition.
   * All fields are nullable, with null indicating that the field should not be overridden.
   */
  ...
  /**
   * Loads planet overrides from a JSON5 file.
   * The expected format is a flat JSON object where keys are planet IDs and values are override objects.
   * Example:
   * {
   *   "cosmichorizons:earth_moon": { "requiredRocketTier": 2, "name": "Moon" },
   *   "cosmichorizons:mars_lands": { "requiredRocketTier": 4, "requiredSuitTag": "chex:suits/suit2" }
   * }
   */
        ...
        for (Map.Entry<String, JsonElement> entry : root.entrySet()) {
          ...
          if (entry.getValue().isJsonObject()) {
            try {
              ...
              System.err.println("Failed to parse override for " + planetId + ": " + e.getMessage());
            }
          }
        }
  ...
  /**
   * Parses a JSON object into a PlanetOverridesCore.Entry object.
   * Handles all possible fields that can be overridden for a planet.
   */
  ...
  /**
   * Parses a string set from a JSON array in the given object.
   * Returns null if the key doesn't exist or the value is not an array.
   */
  ...
  /**
   * Gets a string value from a JSON object, returning null if the key doesn't exist or the value is empty.
   */
