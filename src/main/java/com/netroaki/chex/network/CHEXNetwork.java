...
public class CHEXNetwork {
    ...
    public static void sendPlayerTierToClient(ServerPlayer player) {
        player.getCapability(PlayerTierCapability.INSTANCE).ifPresent(capability -> {
            PlayerTierSyncMessage message = new PlayerTierSyncMessage(player.getUUID(), capability);
     ...
     */
    public static void broadcastProgressionUpdate(ServerPlayer player, String updateType, String data) {
        ProgressionUpdateMessage message = new ProgressionUpdateMessage(player.getUUID(), updateType, data);
