...
public class ProgressionUpdateMessage {
    ...
    public static void handle(ProgressionUpdateMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        context.enqueueWork(() -> {
            if (context.getDirection().getReceptionSide().isClient()) {
                ...
                if (serverPlayer != null) {
                    com.netroaki.chex.CHEX.LOGGER.info("Progression update: {} - {} by {}",
                            message.updateType, message.data, serverPlayer.getGameProfile().getName());
                }
            }
        });
