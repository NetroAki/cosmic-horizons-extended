...
public class MineralDiscoverySystem {
    ...
    public static void discoverMineral(ServerPlayer player, String mineral) {
        player.getCapability(PlayerTierCapability.INSTANCE).ifPresent(capability -> {
            if (!capability.hasDiscoveredMineral(mineral)) {
                ...
                CHEXNetwork.broadcastProgressionUpdate(player, "mineral",
                        playerName + " discovered " + mineral + "!");
     ...
     */
    private static void checkForPlanetUnlocks(ServerPlayer player, PlayerTierCapability capability, String mineral) {
        int mineralTier = GregTechBridge.getMineralGTTier(mineral);

        // Check all planets to see if this mineral discovery unlocks any
        PlanetRegistry.getAllPlanets().values().forEach(planet -> {
            String planetId = planet.id().toString();

            // If player can access the planet's rocket tier and hasn't unlocked it yet
            if (capability.canAccessRocketTier(planet.requiredRocketTier()) &&
                    !capability.hasUnlockedPlanet(planetId)) {

                // Check if this planet has this mineral
                if (GregTechBridge.isMineralAvailableOnPlanet(planetId, mineral)) {
                    capability.unlockPlanet(planetId);

                    // Broadcast the planet unlock
                    String playerName = player.getGameProfile().getName();
                    CHEXNetwork.broadcastProgressionUpdate(player, "planet",
                            playerName + " discovered planet " + planetId + "!");

                    CHEX.LOGGER.info("Player {} unlocked planet {} through mineral discovery",
                            playerName, planetId);
                }
            }
        });
    ...
    private static String getMineralFromBlock(Block block) {
        ...
        if (blockName.contains("iron_ore"))
            return "iron";
        if (blockName.contains("copper_ore"))
            return "copper";
        if (blockName.contains("tin_ore"))
            return "tin";
        if (blockName.contains("silver_ore"))
            return "silver";
        if (blockName.contains("lead_ore"))
            return "lead";
        if (blockName.contains("gold_ore"))
            return "gold";
        if (blockName.contains("nickel_ore"))
            return "nickel";
        if (blockName.contains("cobalt_ore"))
            return "cobalt";
        if (blockName.contains("platinum_ore"))
            return "platinum";
        if (blockName.contains("titanium_ore"))
            return "titanium";
        if (blockName.contains("tungsten_ore"))
            return "tungsten";
        if (blockName.contains("uranium_ore"))
            return "uranium";
        if (blockName.contains("thorium_ore"))
            return "thorium";
    ...
    private static String getMineralFromItem(ItemStack stack) {
        ...
        if (itemName.contains("iron_ingot") || itemName.contains("iron_dust"))
            return "iron";
        if (itemName.contains("copper_ingot") || itemName.contains("copper_dust"))
            return "copper";
        if (itemName.contains("tin_ingot") || itemName.contains("tin_dust"))
            return "tin";
        if (itemName.contains("silver_ingot") || itemName.contains("silver_dust"))
            return "silver";
        if (itemName.contains("lead_ingot") || itemName.contains("lead_dust"))
            return "lead";
        if (itemName.contains("gold_ingot") || itemName.contains("gold_dust"))
            return "gold";
        if (itemName.contains("nickel_ingot") || itemName.contains("nickel_dust"))
            return "nickel";
        if (itemName.contains("cobalt_ingot") || itemName.contains("cobalt_dust"))
            return "cobalt";
        ...
        if (itemName.contains("tungsten_ingot") || itemName.contains("tungsten_dust"))
            return "tungsten";
        if (itemName.contains("uranium_ingot") || itemName.contains("uranium_dust"))
            return "uranium";
        if (itemName.contains("thorium_ingot") || itemName.contains("thorium_dust"))
            return "thorium";
    ...
    public static Set<String> getAvailableMineralsForPlanet(String planetId) {
        Map<String, Object> mineralData = GregTechBridge.getMineralDataForPlanet(planetId);
