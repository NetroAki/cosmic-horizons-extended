...
& magick convert -size 64x64 xc:gray -colorspace gray -blur 0x0.5 -normalize -evaluate sin 3.14 -auto-level $noiseFile
...
foreach ($fluid in $fluids) {
    ...
    & magick convert -size 64x32 xc:none `
        -fill $color -tile $noiseFile -compose Overlay -composite `
    ...
    & magick convert -size 16x16 xc:none `
