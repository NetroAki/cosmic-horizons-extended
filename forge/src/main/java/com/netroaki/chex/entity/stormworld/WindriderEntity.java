...
public class WindriderEntity extends StormEntity implements FlyingAnimal {
  private static final EntityDataAccessor<Boolean> DATA_IS_GLIDING =
      SynchedEntityData.defineId(WindriderEntity.class, EntityDataSerializers.BOOLEAN);
  ...
  public void tick() {
    ...
    if (!this.level().isClientSide) {
      ...
      if (this.tickCount % 20 == 0) {
        this.windDirection =
            StormworldWeatherManager.getWindDirection(this.level(), this.blockPosition());
  ...
  private void spawnGlideParticles() {
    if (this.random.nextInt(5) == 0) {
      ...
      this.level().addParticle(ParticleTypes.CLOUD, d0, d1, d2, 0, -0.1, 0);
    }
  }
  ...
  public boolean canGustAttack() {
    return this.gustCooldown <= 0
        && this.getTarget() != null
        && this.distanceToSqr(this.getTarget()) < 16.0D
      ...
      for (Entity entity : serverLevel.getEntities(this, aabb)) {
        if (entity instanceof LivingEntity && entity.isPushable()) {
          ...
          for (int i = 0; i < 10; i++) {
            ...
            serverLevel.sendParticles(ParticleTypes.CLOUD, d0, d1, d2, 1, 0, 0, 0, 0.1);
          }
        }
      }
  ...
  static class WindriderWanderGoal extends Goal {
    ...
    public void start() {
      double d0 =
          this.windrider.getX() + (this.windrider.getRandom().nextFloat() * 2.0F - 1.0F) * 16.0F;
      double d1 =
          Mth.clamp(
              this.windrider.getY() + (this.windrider.getRandom().nextInt(16) - 8),
              this.windrider.level().getMinBuildHeight() + 8,
              this.windrider.level().getMaxBuildHeight() - 8);
      double d2 =
          this.windrider.getZ() + (this.windrider.getRandom().nextFloat() * 2.0F - 1.0F) * 16.0F;
  ...
  static class WindriderMeleeAttackGoal extends Goal {
    ...
    public WindriderMeleeAttackGoal(
        WindriderEntity mob, double speedModifier, boolean followWhenNotSeen) {
      this.mob = mob;
      this.speedModifier = speedModifier;
    ...
    public void tick() {
      ...
      if ((this.followingTargetEvenIfNotSeen || this.mob.getSensing().hasLineOfSight(target))
          ...
          && (this.pathedTargetX == 0.0D && this.pathedTargetY == 0.0D && this.pathedTargetZ == 0.0D
              || target.distanceToSqr(this.pathedTargetX, this.pathedTargetY, this.pathedTargetZ)
                  >= 1.0D
  ...
  static class GustAttackGoal extends Goal {
    ...
    public boolean canContinueToUse() {
      return this.chargeTime > 0
          && this.windrider.getTarget() != null
          && this.windrider.getTarget().isAlive();
    }
