...
/**
 * Biome registry and bootstrap helpers for CHEX. The palette is still a work in progress, so each
 * biome currently exposes distinctive colours while we iterate on dedicated features and mob rules.
 * Generation/mob settings remain empty placeholders but the structure gives us a single spot to
 * elaborate on once the world-gen JSONs land.
 */
public final class CHEXBiomes {
  ...
  public static final ResourceKey<Biome> NEUTRON_STAR_CRUSHED_BASALT_PLAINS =
      key("neutron_star_crushed_basalt_plains");
  public static final ResourceKey<Biome> SHATTERED_DYSON_HABITAT_DOME =
      key("shattered_dyson_habitat_dome");
  public static final ResourceKey<Biome> CRYSTALIS_PRISM_RIDGES = key("crystalis_prism_ridges");
  public static final ResourceKey<Biome> INFERNO_PRIME_VOLCANIC_PLAINS =
      key("inferno_prime_volcanic_plains");
  public static final ResourceKey<Biome> AQUA_MUNDUS_ABYSSAL_TRENCH =
      key("aqua_mundus_abyssal_trench");
  ...
  public static final ResourceKey<Biome> ARRAKIS_SIETCH_STRONGHOLDS =
      key("arrakis_sietch_strongholds");
  public static final ResourceKey<Biome> ARRAKIS_STORMLANDS = key("arrakis_stormlands");

  public static final ResourceKey<Biome> PANDORA_BIOLUMINESCENT_FOREST =
      key("pandora_bioluminescent_forest");
  public static final ResourceKey<Biome> PANDORA_FLOATING_MOUNTAINS =
      key("pandora_floating_mountains");
  public static final ResourceKey<Biome> PANDORA_OCEAN_DEPTHS = key("pandora_ocean_depths");
  public static final ResourceKey<Biome> PANDORA_VOLCANIC_WASTELAND =
      key("pandora_volcanic_wasteland");
  ...
  private static ResourceKey<Biome> key(String path) {
    return ResourceKey.create(
        Registries.BIOME, ResourceLocation.fromNamespaceAndPath(CHEX.MOD_ID, path));
  }
  ...
  public static void registerRegions() {
    if (CHEXConfig.enableTerraBlenderOverlay()) {
      Regions.register(
          new CHEXRegion(ResourceLocation.fromNamespaceAndPath(CHEX.MOD_ID, "chex_region"), 10));
  ...
  private static Biome buildBiome(
      float temperature, float downfall, BiomeSpecialEffects.Builder effectsBuilder) {
