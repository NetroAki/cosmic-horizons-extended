...
public class CHEXRegion extends Region {
  ...
  public void addBiomes(
    ...
    new ParameterPointListBuilder()
        ...
        .build()
        .forEach(point -> builder.add(point, CHEXBiomes.PANDORA_SKY_ISLANDS));

    // Add our points to the mapper
    builder.build().forEach(mapper::accept);

    // Register simple surface rules to give each active biome a unique top block
    SurfaceRules.RuleSource arrakisDunesTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.ARRAKIS_GREAT_DUNES),
                SurfaceRules.state(CHEXBlocks.ARRAKIS_SAND.get().defaultBlockState())));

    SurfaceRules.RuleSource arrakisSpiceTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.ARRAKIS_SPICE_MINES),
                SurfaceRules.state(CHEXBlocks.ARRAKITE_SANDSTONE.get().defaultBlockState())));

    SurfaceRules.RuleSource arrakisPolarTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.ARRAKIS_POLAR_ICE_CAPS),
                SurfaceRules.state(CHEXBlocks.ARRAKIS_SALT.get().defaultBlockState())));

    SurfaceRules.RuleSource arrakisSietchTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.ARRAKIS_SIETCH_STRONGHOLDS),
                SurfaceRules.state(CHEXBlocks.ARRAKIS_ROCK.get().defaultBlockState())));

    SurfaceRules.RuleSource arrakisStormTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.ARRAKIS_STORMLANDS),
                SurfaceRules.state(CHEXBlocks.INFERNO_ASH.get().defaultBlockState())));

    SurfaceRules.RuleSource pandoraForestTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.PANDORA_BIOLUMINESCENT_FOREST),
                SurfaceRules.state(CHEXBlocks.PANDORA_GRASS.get().defaultBlockState())));
    SurfaceRules.RuleSource pandoraFloatingTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.PANDORA_FLOATING_MOUNTAINS),
                SurfaceRules.state(CHEXBlocks.PANDORA_BLOOM.get().defaultBlockState())));
    SurfaceRules.RuleSource pandoraOceanTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.PANDORA_OCEAN_DEPTHS),
                SurfaceRules.state(CHEXBlocks.AQUA_MUNDUS_STONE.get().defaultBlockState())));
    SurfaceRules.RuleSource pandoraVolcanicTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.PANDORA_VOLCANIC_WASTELAND),
                SurfaceRules.state(CHEXBlocks.INFERNO_STONE.get().defaultBlockState())));
    SurfaceRules.RuleSource pandoraSkyTop =
        SurfaceRules.sequence(
            SurfaceRules.ifTrue(
                SurfaceRules.isBiome(CHEXBiomes.PANDORA_SKY_ISLANDS),
                SurfaceRules.state(CHEXBlocks.PANDORA_GRASS.get().defaultBlockState())));
