...
public class CHEXWorldGenProvider {
  ...
  private static void createMineralFeature(
    ...
    var mineralData = com.netroaki.chex.CHEX.gt().getMineralDataForPlanet(planetId.toString());
  ...
  private static void createOreFeature(
      BootstapContext<ConfiguredFeature<?, ?>> context,
      ResourceLocation planetId,
      String mineral,
      int tier,
    ...
    CHEX.LOGGER.debug(
        "Would create ore feature for {} on {}: tier={}, abundance={}, depth={}-{}",
        mineral,
        planetId,
        tier,
        abundance,
    ...
    CHEX.LOGGER.debug(
        "Calculated vein size: {} for mineral {} on planet {}", veinSize, mineral, planetId);
  }
  ...
  private static void createPlacedMineralFeature(
  ...
  private static void createPlanetBiome(
      BootstapContext<Biome> context, ResourceLocation planetId, PlanetDef planet) {
  ...
  private static void createPlanetNoiseSettings(
      BootstapContext<NoiseGeneratorSettings> context,
      ResourceLocation planetId,
      PlanetDef planet) {
    // Placeholder implementation - would create custom terrain generation
    CHEX.LOGGER.debug(
        "Creating noise settings for planet {} with gravity {}", planetId, planet.gravityLevel());
  ...
  public static ResourceKey<ConfiguredFeature<?, ?>> getPlanetMineralFeatureKey(
      ResourceLocation planetId, String mineral) {
    return ResourceKey.create(
        Registries.CONFIGURED_FEATURE, CHEX.id(planetId.getPath() + "_" + mineral + "_ore"));
  }

  /** Get resource key for a planet's placed feature */
  public static ResourceKey<PlacedFeature> getPlanetMineralPlacedFeatureKey(
      ResourceLocation planetId, String mineral) {
    return ResourceKey.create(
        Registries.PLACED_FEATURE, CHEX.id(planetId.getPath() + "_" + mineral + "_ore_placed"));
  }

  /** Get resource key for a planet's biome */
  public static ResourceKey<Biome> getPlanetBiomeKey(ResourceLocation planetId) {
    return ResourceKey.create(Registries.BIOME, CHEX.id(planetId.getPath() + "_biome"));
  }

  /** Get resource key for a planet's noise settings */
  public static ResourceKey<NoiseGeneratorSettings> getPlanetNoiseSettingsKey(
      ResourceLocation planetId) {
    return ResourceKey.create(Registries.NOISE_SETTINGS, CHEX.id(planetId.getPath() + "_noise"));
  }
}
