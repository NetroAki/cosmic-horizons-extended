...
/**
 * Simple ring strip generator: - Inside |z| <= BAND_HALF_WIDTH: delegate to vanilla overworld
 * noise. - At |z| == BAND_HALF_WIDTH: build black wall columns up to WALL_HEIGHT. - Outside |z| >
 * BAND_HALF_WIDTH: generate void (air) with bedrock floor at y=-64.
 */
public class StripRingGenerator extends ChunkGenerator {

  public static final Codec<StripRingGenerator> CODEC =
      RecordCodecBuilder.create(
          instance ->
              instance
                  .group(ChunkGenerator.CODEC.fieldOf("inner").forGetter(g -> g.inner))
  ...
  private void addAlienFlora(WorldGenLevel level, ChunkAccess chunk) {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      // Find surface
      BlockPos surfacePos =
          level.getHeightmapPos(Heightmap.Types.WORLD_SURFACE_WG, new BlockPos(x, 0, z));
  ...
  private void applyTerrainLift(ChunkAccess chunk, int minY, int maxY) {
    ...
    if (foundTerrain) {
      System.out.println(
          "StripRingGenerator: Applied terrain lift to "
              + terrainCount
              + " columns in chunk "
              + chunk.getPos());
    } else {
  ...
  private void enforceInsideBandBasement(ChunkAccess chunk, int minY, int maxY) {
    ...
    int minZ = chunk.getPos().getMinBlockZ();
    int maxZ = minZ + 15;
    if (Math.max(Math.abs(minZ), Math.abs(maxZ)) > BAND_HALF_WIDTH) return;
    for (int lx = 0; lx < 16; lx++) {
      int worldX = chunk.getPos().getMinBlockX() + lx;
      for (int lz = 0; lz < 16; lz++) {
        int worldZ = chunk.getPos().getMinBlockZ() + lz;
        if (Math.abs(worldZ) > BAND_HALF_WIDTH) continue;
