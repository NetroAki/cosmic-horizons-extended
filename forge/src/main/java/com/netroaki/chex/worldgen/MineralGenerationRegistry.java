...
public final class MineralGenerationRegistry {
  ...
  private static final Map<ResourceLocation, List<Holder<PlacedFeature>>> FEATURES_BY_BIOME =
      new HashMap<>();
  private static final Map<ResourceLocation, List<Holder<PlacedFeature>>> FEATURES_BY_TAG =
      new HashMap<>();
  ...
  public static void reload() {
    ...
    if (mineralsConfig.planets().isEmpty()) {
      CHEX.LOGGER.info(
          "[CHEX] Minerals config contained no planet entries - skipping mineral feature"
              + " registration.");
      initialized = false;
      return;
    }
    ...
    mineralsConfig
        .planets()
        .forEach(
            (planetId, planetEntry) -> {
              planetEntry.mineralTiers.forEach(
                  tierEntry ->
                      tierEntry.distributions.forEach(
                          dist -> {
                            Holder<PlacedFeature> placed =
                                buildPlacedFeature(dist, hasGT, fallback, blockTags, planetId);
                            if (placed == null) {
                              return;
                            }

                            String descriptor =
                                dist.tag != null && !dist.tag.isBlank() ? dist.tag : dist.block;
                            if (dist.biomes == null || dist.biomes.isEmpty()) {
                              CHEX.LOGGER.warn(
                                  "[CHEX] Mineral distribution {} for planet {} has no biomes"
                                      + " specified; skipping.",
                                  descriptor,
                                  planetId);
                              return;
                            ...
                            for (String biomeId : dist.biomes) {
                              ...
                              if (loc == null) {
                                CHEX.LOGGER.warn(
                                    "[CHEX] Invalid biome identifier '{}' in minerals config;"
                                        + " skipping.",
                                    biomeId);
                                continue;
                              }
                              if (isTagRef) {
                                FEATURES_BY_TAG
                                    .computeIfAbsent(loc, rl -> new ArrayList<>())
                                    .add(placed);
                              } else {
                                FEATURES_BY_BIOME
                                    .computeIfAbsent(loc, rl -> new ArrayList<>())
    ...
    CHEX.LOGGER.info(
        "[CHEX] Mineral generation ready: {} placed feature entries across {} biomes and {} tags"
            + " (GTCEu={}).",
        distributionCount,
        FEATURES_BY_BIOME.size(),
        FEATURES_BY_TAG.size(),
        hasGT);
  }
  ...
  public static List<Holder<PlacedFeature>> gatherFeatures(
      ResourceLocation biomeId, java.util.Collection<ResourceLocation> tags) {
    if (!initialized) {
      return List.of();
    }
  ...
  private static Holder<PlacedFeature> buildPlacedFeature(
      MineralsConfig.Distribution dist,
      boolean hasGT,
      FallbackOres fallback,
    ...
    if (!hasTag && !hasBlock) {
      CHEX.LOGGER.warn(
          "[CHEX] Mineral distribution on {} missing tag/block entry; skipping.", planetId);
      return null;
    }
    ...
    if (oreState == null) {
      CHEX.LOGGER.warn(
          "[CHEX] Unable to resolve ore block for '{}' (planet {}).", descriptor, planetId);
      return null;
    }
    ...
    ConfiguredFeature<OreConfiguration, ?> configured =
        new ConfiguredFeature<>(Feature.ORE, oreConfig);
  ...
  private static BlockState resolveOreState(
      MineralsConfig.Distribution dist,
      boolean hasGT,
      FallbackOres fallback,
      ITagManager<Block> blockTags) {
    if (dist.block != null && !dist.block.isBlank()) {
      ...
      if (blockLoc == null) {
        ...
        CHEX.LOGGER.warn(
            "[CHEX] Ore block '{}' not found, falling back to tag handling.", dist.block);
      }
    }
