...
public class AureliaFloraGenerator extends Feature<AureliaFloraGenerator.Config> {
    ...
    public static final Codec<AureliaFloraGenerator.Config> CODEC = RecordCodecBuilder.create(instance ->
        instance.group(
            Codec.intRange(0, 100).fieldOf("tries").orElse(64).forGetter(config -> config.tries),
            Codec.intRange(1, 16).fieldOf("xz_spread").orElse(6).forGetter(config -> config.xzSpread),
            Codec.intRange(1, 8).fieldOf("y_spread").orElse(1).forGetter(config -> config.ySpread)
