...
public class StormRocEntity extends Monster {
  ...
  public void tick() {
    ...
    if (!this.level().isClientSide) {
      ...
      if (this.wingFlapCooldown > 0) this.wingFlapCooldown--;
    }
  }
  ...
  private void performLightningAttack() {
    if (this.getTarget() != null) {
      ...
      spawnLightningAt(targetPos);

      // Additional strikes in enraged phase
      if (this.phase == 2) {
        for (int i = 0; i < 3; i++) {
          BlockPos offsetPos =
              targetPos.offset(this.random.nextInt(5) - 2, 0, this.random.nextInt(5) - 2);
          spawnLightningAt(offsetPos);
        }
      }
    }
  }
  ...
  private void performWingFlap() {
    ...
    for (Player player :
        this.level()
            .getEntitiesOfClass(Player.class, this.getBoundingBox().inflate(8.0D, 4.0D, 8.0D))) {
