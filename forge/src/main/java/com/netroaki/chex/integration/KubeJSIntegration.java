...
/**
 * Integration with KubeJS for recipe gating and tier-based conditions. This provides
 * JavaScript-accessible functions for checking player tiers.
 */
public class KubeJSIntegration {

  /**
   * Check if a player has unlocked a specific rocket tier Usage in KubeJS:
   * PlayerTier.hasRocketTier(player, tier)
   */
  public static boolean hasRocketTier(Player player, int tier) {
    if (player == null || tier < 1 || tier > 10) return false;
  ...
  /**
   * Check if a player has unlocked a specific suit tier Usage in KubeJS:
   * PlayerTier.hasSuitTier(player, tier)
   */
  public static boolean hasSuitTier(Player player, int tier) {
    if (player == null || tier < 1 || tier > 5) return false;
  ...
  /** Get a player's current rocket tier Usage in KubeJS: PlayerTier.getRocketTier(player) */
  ...
  /** Get a player's current suit tier Usage in KubeJS: PlayerTier.getSuitTier(player) */
  ...
  /**
   * Check if a player has discovered a specific planet Usage in KubeJS:
   * PlayerTier.hasDiscoveredPlanet(player, planetId)
   */
  public static boolean hasDiscoveredPlanet(Player player, String planetId) {
    if (player == null || planetId == null) return false;
  ...
  /**
   * Check if a player has discovered a specific mineral Usage in KubeJS:
   * PlayerTier.hasDiscoveredMineral(player, mineralId)
   */
  public static boolean hasDiscoveredMineral(Player player, String mineralId) {
    if (player == null || mineralId == null) return false;
  ...
  /** Unlock a planet for a player Usage in KubeJS: PlayerTier.unlockPlanet(player, planetId) */
  public static boolean unlockPlanet(Player player, String planetId) {
    ...
    CHEX.LOGGER.info(
        "Unlocked planet {} for player {}", planetId, player.getGameProfile().getName());
    return true;
  }

  /** Unlock a mineral for a player Usage in KubeJS: PlayerTier.unlockMineral(player, mineralId) */
  public static boolean unlockMineral(Player player, String mineralId) {
    ...
    CHEX.LOGGER.info(
        "Discovered mineral {} for player {}", mineralId, player.getGameProfile().getName());
    return true;
  }

  /**
   * Unlock a rocket tier for a player Usage in KubeJS: PlayerTier.unlockRocketTier(player, tier)
   */
  public static boolean unlockRocketTier(Player player, int tier) {
    ...
    if (success) {
      CHEX.LOGGER.info(
          "Unlocked nodule tier T{} for player {}", tier, player.getGameProfile().getName());
    }
    return success;
  }
  ...
  /**
   * Get the nodule material theme for a tier Usage in KubeJS: PlayerTier.getNoduleMaterial(tier)
   */
  ...
  /**
   * Get the current player's nodule material theme based on their tier Usage in KubeJS:
   * PlayerTier.getPlayerNoduleMaterial(player)
   */
  ...
  /** Unlock a suit tier for a player Usage in KubeJS: PlayerTier.unlockSuitTier(player, tier) */
  ...
  /**
   * Get all discovered planets for a player Usage in KubeJS:
   * PlayerTier.getDiscoveredPlanets(player)
   */
  public static String[] getDiscoveredPlanets(Player player) {
    ...
    PlayerTierCapability capability =
        player.getCapability(PlayerTierCapability.INSTANCE).orElse(null);
    if (capability == null) return new String[0];

    return capability.getUnlockedPlanets().toArray(new String[0]);
  }

  /**
   * Get all discovered minerals for a player Usage in KubeJS:
   * PlayerTier.getDiscoveredMinerals(player)
   */
  ...
  /**
   * Check if a player can access a specific planet based on their rocket tier Usage in KubeJS:
   * PlayerTier.canAccessPlanet(player, planetId)
   */
  public static boolean canAccessPlanet(Player player, String planetId) {
    ...
    if (!capability.hasUnlockedPlanet(planetId)) return false;
  ...
  /**
   * Get the required rocket tier for a planet Usage in KubeJS:
   * PlayerTier.getRequiredRocketTier(planetId)
   */
  ...
  /**
   * Get the required suit tier for a planet Usage in KubeJS:
   * PlayerTier.getRequiredSuitTier(planetId)
   */
  ...
  public static void register() {
    CHEX.LOGGER.info(
        "KubeJS integration registered - tier checking and unlocking functions available in"
            + " JavaScript");
