...
public final class ConfigReloader {
  ...
  public static void reloadAll() {
    ...
    var minerals =
        com.netroaki.chex.config.MineralsConfigCore.load(cfg.resolve("chex/chex-minerals.json5"));
    CHEX.LOGGER.info("[CHEX] Minerals config {}", minerals.isPresent() ? "loaded" : "not found");

    // Fallback ores (fallback_ores.json5)
    var fallback =
        com.netroaki.chex.config.FallbackOresCore.load(cfg.resolve("chex/fallback_ores.json5"));
    CHEX.LOGGER.info(
        "[CHEX] Fallback ores {} ({} entries)",
        fallback.isPresent() ? "loaded" : "not found",
        fallback.map(java.util.Map::size).orElse(0));

    // Planet overrides (chex-planets.json5)
    var overrides =
        com.netroaki.chex.config.PlanetOverridesCore.load(cfg.resolve("chex/chex-planets.json5"));
    CHEX.LOGGER.info(
        "[CHEX] Planet overrides {} ({} entries)",
