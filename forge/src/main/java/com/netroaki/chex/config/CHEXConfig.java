...
public final class CHEXConfig {
  ...
  private static final Map<NoduleTiers, ForgeConfigSpec.ConfigValue<String>> FUEL_BY_TIER =
      new HashMap<>();
  ...
  private static final Map<NoduleTiers, ForgeConfigSpec.IntValue> FUEL_VOLUME_BY_TIER =
      new HashMap<>();
  ...
  public static void register() {
    ...
    ModLoadingContext.get()
        .registerConfig(ModConfig.Type.COMMON, COMMON_SPEC, CHEX.MOD_ID + "-common.toml");
  }

  private static void defineFuel(
      ForgeConfigSpec.Builder builder, NoduleTiers tier, String defaultFluid) {
    FUEL_BY_TIER.put(
        tier,
        builder.comment("Fluid ID for tier " + tier.getTier()).define(tier.name(), defaultFluid));
  }
  ...
  private static void defineFuelVolume(
      ForgeConfigSpec.Builder builder, NoduleTiers tier, int defaultMb) {
    FUEL_VOLUME_BY_TIER.put(
        tier,
        builder
            .comment("Fuel volume (mB) for tier " + tier.getTier())
