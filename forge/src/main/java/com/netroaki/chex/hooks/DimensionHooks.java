...
public final class DimensionHooks {
    ...
    private boolean hasRadiationHazard(ResourceKey<Level> dimension) {
        ...
        if (def != null) {
            ...
            if (tier >= 8)
                return true;
            if (biomeType.contains("stellar") || biomeType.contains("gas_giant") || biomeType.contains("ring_system")
                    || biomeType.contains("extreme") || biomeType.contains("ice_giant"))
                return true;
            return false;
        }
     ...
     */
    private void handleInsufficientSuit(Player player, ResourceKey<Level> dimension, SuitTiers required,
            SuitTiers current) {
            ...
            if (server != null) {
                ...
                if (overworld != null) {
                    ...
                    CHEX.LOGGER.info(
                            ...
                            player.getGameProfile().getName(), dimensionName, requiredName, currentName);
                }
            }
        } else {
            // No hypoxia; oxygen handling is external. Just notify.
            CHEX.LOGGER.info("Insufficient suit for {} (required: {}, current: {}), not bouncing due to config",
                    dimensionName, requiredName, currentName);
        }
    }
