...
public class RingworldWallHooks {
    ...
    public static void onChunkLoad(ChunkEvent.Load event) {
        ...
        if (!level.dimension().location().toString().equals("cosmic_horizons_extended:aurelia_ringworld")) return;
    ...
    private static void generateWallInChunk(ServerLevel level, LevelChunk chunk, double distance) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                double blockDistance = Math.sqrt(worldPos.getX() * worldPos.getX() + worldPos.getZ() * worldPos.getZ());
    ...
    private static void generateWallColumn(ServerLevel level, BlockPos pos, double distance) {
        ...
        for (int y = level.getMinBuildHeight(); y < level.getMaxBuildHeight(); y++) {
            ...
            if (currentState.isAir() || currentState.canBeReplaced()) {
                BlockState wallState = CHEXBlocks.RINGWORLD_WALL.get().defaultBlockState()
                ...
                if (level.getBlockEntity(wallPos) instanceof com.netroaki.chex.blocks.RingworldWallBlockEntity wallEntity) {
