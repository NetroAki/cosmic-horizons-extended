...
public final class RingworldWrapHooks {
    ...
    public void onPlayerTick(final TickEvent.PlayerTickEvent event) {
        ...
        double canonical = ((x + HALF) % RING_CIRCUMFERENCE + RING_CIRCUMFERENCE) % RING_CIRCUMFERENCE - HALF;
        if (canonical != x) {
            serverPlayer.teleportTo(level, canonical, y, z, Set.copyOf(EnumSet.noneOf(RelativeMovement.class)), serverPlayer.getYRot(), serverPlayer.getXRot());
        }
    }
}
