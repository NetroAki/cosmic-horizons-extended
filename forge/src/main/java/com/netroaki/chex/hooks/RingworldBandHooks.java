...
/**
 * Enforces the ring band: clears blocks outside the band to air and builds
 * black wall columns at the band edge up to a fixed height. Works with the
 * datapack-driven overworld generator by post-processing chunks on load.
 */
public final class RingworldBandHooks {
    ...
    private static final Map<ResourceKey<Level>, Set<Long>> PROCESSED_BY_DIM = new ConcurrentHashMap<>();
    private static final Map<ResourceKey<Level>, java.util.ArrayDeque<long[]>> PENDING_COLUMNS = new ConcurrentHashMap<>();
    ...
    @SubscribeEvent
    public void onChunkLoad(final ChunkEvent.Load event) { /* no-op; work is triggered on watch */ }
    ...
    public void onChunkWatch(final net.minecraftforge.event.level.ChunkWatchEvent.Watch event) {
        ...
        if (!serverLevel.dimension().location().equals(AURELIA_RINGWORLD.location()))
            return;
        ...
        final Set<Long> processed = PROCESSED_BY_DIM.computeIfAbsent(serverLevel.dimension(),
        ...
        for (int lx = 0; lx < 16; lx++) {
            ...
            for (int lz = 0; lz < 16; lz++) {
                ...
                if (absZ > BAND_HALF_WIDTH) {
                    PENDING_COLUMNS.computeIfAbsent(serverLevel.dimension(), d -> new java.util.ArrayDeque<>())
        ...
        if (enqueued > 0) {
            com.netroaki.chex.CHEX.LOGGER.info("[RingBand] Watched chunk {},{}: enqueued {} columns", posChunk.x,
                    posChunk.z, enqueued);
        }
    }
    ...
    public void onLevelTick(net.minecraftforge.event.TickEvent.LevelTickEvent e) {
        if (e.phase != net.minecraftforge.event.TickEvent.Phase.END)
            return;
        if (!(e.level instanceof net.minecraft.server.level.ServerLevel serverLevel))
            return;
        ...
        if (processed > 0) {
            com.netroaki.chex.CHEX.LOGGER.info("[RingBand] Culled {} columns this tick in {}", processed,
    ...
    public void onPlayerTick(net.minecraftforge.event.TickEvent.PlayerTickEvent e) {
        ...
        final int minY = serverLevel.getMinBuildHeight();
        final int maxY = serverLevel.getMaxBuildHeight();
        final Set<Long> processed = PROCESSED_BY_DIM.computeIfAbsent(serverLevel.dimension(), d -> ConcurrentHashMap.newKeySet());
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dz = -radius; dz <= radius; dz++) {
                ...
                if (Math.min(Math.abs(minZ), Math.abs(minZ + 15)) <= BAND_HALF_WIDTH && maxAbsZ >= BAND_HALF_WIDTH) {
                    final BlockPos.MutableBlockPos bp = new BlockPos.MutableBlockPos();
                    final BlockState wall = CHEXBlocks.AURELIA_WALL.get().defaultBlockState();
                    int wallTop = Math.min(WALL_HEIGHT, maxY - 1);
