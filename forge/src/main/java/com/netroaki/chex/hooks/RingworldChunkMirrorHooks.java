...
/**
 * Mirrors chunk contents along X so that chunks beyond the circumference reuse
 * the canonical chunk data.
 * This ensures edits at the seam persist across the loop without relying on
 * player teleport wrapping.
 */
public final class RingworldChunkMirrorHooks {

    private static final double RING_CIRCUMFERENCE = 2000.0; // must match wrap hooks and gameplay expectations
    ...
    public void onChunkLoad(final ChunkEvent.Load event) {
        ...
        if (!serverLevel.dimension().location().equals(AURELIA_RINGWORLD.location()))
            return;
        if (!(event.getChunk() instanceof LevelChunk targetChunk))
            return;
