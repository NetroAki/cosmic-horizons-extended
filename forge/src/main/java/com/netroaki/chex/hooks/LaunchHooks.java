...
public final class LaunchHooks {
    ...
    public static boolean canLaunch(Player player, ResourceKey<Level> target, int rocketTierLevel) {
        ...
        if (!TravelGraph.isPlanetAccessible(target.location(), rocketTierLevel)) {
            ...
            CHEXNetwork.sendTo((net.minecraft.server.level.ServerPlayer) player,
                    new LaunchDenyMessage(LaunchDenyMessage.Code.TIER,
        ...
        if (!playerTier.canAccessNoduleTier(NoduleTiers.getByTier(rocketTierLevel))) {
            CHEXNetwork.sendTo((net.minecraft.server.level.ServerPlayer) player,
                    new LaunchDenyMessage(LaunchDenyMessage.Code.TIER,
                            "Nodule tier not unlocked: T" + rocketTierLevel));
            return false;
        }
        ...
        if (!playerTier.hasUnlockedPlanet(target.location().toString())) {
            CHEXNetwork.sendTo((net.minecraft.server.level.ServerPlayer) player,
                    new LaunchDenyMessage(LaunchDenyMessage.Code.DISCOVERY,
        ...
        CHEX.LOGGER.info("Launch validation passed: player={}, target={}, tier={}",
    ...
    /**
     * Consume fuel buckets from the player's inventory for a successful launch.
     * Returns true if consumption succeeded or was not required.
     */
    public static boolean consumeFuelForLaunch(Player player, int rocketTier) {
        ...
        if (toConsume > 0) {
            CHEX.LOGGER.warn(
                    "[CHEX] Fuel consumption shortfall: neededBuckets={}, remaining={} (should not happen after validation)",
                    neededBuckets, toConsume);
        }
        return true;
    }
    ...
    private static boolean validateSuitRequirements(Player player, ResourceKey<Level> target) {
        ...
        if (playerSuit == null) {
            player.displayClientMessage(
                    Component.literal("§c[CHEX] You need a " + requiredSuit.getDisplayName() + " suit to travel here!"),
                    true);
            return false;
        }
        ...
        if (playerSuit.getTier() < requiredSuit.getTier()) {
            player.displayClientMessage(
                    Component.literal("§c[CHEX] Suit tier too low! Required: " + requiredSuit.getDisplayName() +
                            " (Tier " + requiredSuit.getTier() + "), Current: " + playerSuit.getDisplayName() +
                            " (Tier " + playerSuit.getTier() + ")"),
                    true);
            return false;
        }
    ...
    private static boolean validateFuelRequirements(Player player, int rocketTier) {
        ...
        // fuel
        CHEX.LOGGER.info("Fuel validation: player={}, requiredFuel={}, requiredVolume={}mB, noduleTier={}",
                player.getGameProfile().getName(), fuelName, requiredVolume, rocketTier);
        ...
        if (requiredBucket != null) {
            ...
            if (have < neededBuckets) {
                CHEXNetwork.sendTo((net.minecraft.server.level.ServerPlayer) player,
                        new LaunchDenyMessage(LaunchDenyMessage.Code.FUEL,
