...
/**
 * Network message for syncing player tier capabilities between client and
 * server
 */
public class PlayerTierSyncMessage {
    ...
    public static void handle(PlayerTierSyncMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
        NetworkEvent.Context context = contextSupplier.get();
        context.enqueueWork(() -> {
            if (context.getDirection().getReceptionSide().isClient()) {
                // Handle on client side
                Player player = context.getSender() != null ? context.getSender()
                ...
                if (serverPlayer != null && serverPlayer.getUUID().equals(message.playerId)) {
                    serverPlayer.getCapability(PlayerTierCapability.INSTANCE).ifPresent(capability -> {
                        capability.deserializeNBT(message.capability.serializeNBT());
                    });
                }
            }
