...
public final class CHEXWorldgenProvider implements DataProvider {
  ...
  public java.util.concurrent.CompletableFuture<?> run(CachedOutput cache) {
    ...
    minerals.ifPresent(
        minMap -> {
          for (Map.Entry<String, java.util.List<MineralsConfigCore.DistributionEntry>> planet :
              minMap.entrySet()) {
            ...
            for (MineralsConfigCore.DistributionEntry dist : planet.getValue()) {
              ...
              ResourceLocation configuredId =
                  ResourceLocation.fromNamespaceAndPath(CHEX.MOD_ID, featureName);
              ResourceLocation placedId =
                  ResourceLocation.fromNamespaceAndPath(CHEX.MOD_ID, placedName);

              Map<String, Object> configured =
                  createConfiguredFeatureJson(hasGT ? oreKey : fallback.getBlockForTag(oreKey));
              Map<String, Object> placed =
                  createPlacedFeatureJson(
                      configuredId.toString(), dist.count, dist.minY, dist.maxY);
              ...
              Map<String, Object> modifier =
                  createBiomeModifierJson(placedId.toString(), dist.biomes);
              ResourceLocation modifierId =
                  ResourceLocation.fromNamespaceAndPath(CHEX.MOD_ID, featureName + ";biome_mod");
              Path modifierPath =
                  root.resolve(
                      "data/"
                          + CHEX.MOD_ID
  ...
  private static Map<String, Object> createPlacedFeatureJson(
      String configuredId, int count, int minY, int maxY) {
    Map<String, Object> root = new HashMap<>();
        ...
        java.util.List.of(
            ...
            Map.of(
                ...
                Map.of(
                    ...
                    "minecraft:uniform",
                    "min_inclusive",
                    Map.of("absolute", minY),
                    "max_inclusive",
                    Map.of("absolute", maxY))),
            Map.of("type", "minecraft:biome")));
    return root;
  }

  private static Map<String, Object> createBiomeModifierJson(
      String placedId, java.util.List<String> biomes) {
    Map<String, Object> root = new HashMap<>();
